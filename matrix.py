...
def log_matrix_operation(operation):
    def decorator(func):
        def wrapper(*args, **kwargs):
            if operation == "swap":
                print("r{} ↔ r{}".format(args[1]+1, args[2]+1))
            elif operation == "multiply":
                print("R{} ← R{} * {}".format(args[1]+1, args[1]+1, args[2]))
            elif operation == "add":
                print(
                    "R{} ← R{} + R{}".format(args[1]+1, args[1]+1, args[2]+1))
            elif operation == "m_row_add":
                print(
                    "R{} ← R{} + ({} * R{})".format(args[1]+1, args[1]+1, args[2], args[3]+1))
            elif operation == "sub":
                print(
                    "R{} ← R{} - R{}".format(args[1]+1, args[1]+1, args[2]+1))
            elif operation == "div":
...
class Matrix:
    ...
    def __add__(self, other):
        ...
        for i in range(len(A)):
            for j in range(len(A[0])):
                ...
                display[i][j] = display[i][j].replace(
    ...
    def __mul_scalar(self, other: (int, float)):
        """Multiply matrix by a scalar."""
        return Matrix([[other * self.matrix[i][j] for j in range(self.n)] for i in range(self.m)])
    ...
    def reshape(self, m, n):
        ...
        if self.m * self.n != m * n:
            print("Cannot reshape {}x{} matrix to {}x{} matrix.".format(
                self.m, self.n, m, n))
            return None
        flattened = [item for sublist in self.matrix for item in sublist]
        reshaped = [[flattened[i * n + j] for j in range(n)] for i in range(m)]
    ...
    def determinant(self):
        ...
        for j in range(len(self.matrix)):
            ...
            det += ((-1) ** j) * \
                self.matrix[0][j] * self.determinant(sub_matrix)
        return det
    ...
    def inverse(self):
        ...
        adjugate = Matrix(list(map(list, zip(*cofactors))))
        ...
        for j in range(m.cols):
            ...
            for i in range(j+1, m.rows):
                if m.matrix[i][j] != 0:
                    ...
                    print(
                        f"Making zero at position {i},{j} by subtracting {factor:.2f} times row {j} from row {i}")
    ...
    def determinant_old(self):
        ...
        det = 0
        for j in range(self.n):
            det += ((-1) ** j) * self.matrix[0][j] * \
                self.submatrix(0, j).determinant()
    ...
    def gaussian_elimination(self):
        ...
        for f in free_vars:
            comments.append(
                f"The {f+1} column contains no pivots, thus x_{f+1} is a free variable and the system has infinitely many solutions")
        ...
        for i in range(m):
            if all([cell == 0 for cell in matrix[i][:n]]) and matrix[i][last_col-1] != 0:
                ...
                comments.append(
                    f"The last row is equivalent to 0={matrix[i][last_col-1]}, thus the system is inconsistent")
                return 0, solutions, comments

        if not free_vars:
            comments.append(
                "There are no free variables and the system has a unique solution")
    ...
    def _row_divide(self, i, val=1):
        """Divide row i by the value val."""
        self[i] = [self.matrix[i][j] / val for j in range(self.n)]
        return self

    @log_matrix_operation("multiply")
    def _row_multiply(self, i, val=1):
        # self[i] = [self.matrix[i][j] * val for j in range(self.n)]
        self[i] = [x * val for x in self.matrix[i]]
        return self

    @log_matrix_operation("sub")
    def _row_subtract(self, i, j, factor=1):
        """Subtract factor times row j from row i."""
        # self[i] = [self.matrix[i][k] - factor * self.matrix[j][k] for k in range(self.n)]
        self[i] = [x - factor * y for x,
                   y in zip(self.matrix[i], self.matrix[j])]
        return self

    @log_matrix_operation("m_row_add")
    def _row_add(self, i, j, factor=1):
    ...
    def lu_factorization(self):
        ...
        for j in range(n):
            ...
            for i in range(j+1, n):
                if U.matrix[j][j] == 0:
                    raise ValueError(
                        "LU decomposition is not possible as the matrix is singular.")
    ...
    def forward_substitution(self, L, b):
        ...
        for i in range(1, n):
            y[i] = (b[i] - sum(L.matrix[i][j] * y[j]
                    for j in range(i))) / L.matrix[i][i]
        return y
    ...
    def backward_substitution(self, U, y):
        ...
        for i in range(n-2, -1, -1):
            x[i] = (y[i] - sum(U.matrix[i][j] * x[j]
                    for j in range(i+1, n))) / U.matrix[i][i]
        return x
