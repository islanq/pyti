class Matrix:
  ...
  def __getitem__(self, indices):
    if isinstance(indices, int):  # Single index provided
        ...
        raise IndexError("Matrix index out of range {0}".format(indices))
    
  def __setitem__(self, indices, value):
    if isinstance(indices, tuple):
        if len(indices) != 2:
            raise ValueError("Expected two indices in the form of a tuple (row, col) and received {0}".format(indices))
        row, col = indices
  ...
  def __iter__(self):
    # Reset the current row and column indices
  ...
  def reshape(self, m, n):
    ...
    # Flatten the matrix
    flattened = [item for item in self]
  ...
  def __row_subtract(self, i, j, factor):
    formatting = lambda x: "{:6.2f}".format(x)
    ...
    for row in self.matrix:
        formatted_row = [formatting(val) for val in row]
...
def flatten(lst):
    ...
    for item in lst:
        if isinstance(item, list):
            flattened.extend(flatten(item))  # Recursive call to flatten nested lists
        else:
