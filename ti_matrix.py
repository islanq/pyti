...
if sys.platform == 'TI-Nspire':
    ...
    from ti_interop import TiInterop
    call_func = TiInterop.call_func
    eval_expr = TiInterop.eval_expr
    
class TiMatrix(TiInterop):
    
    def abs(self, py_matrix):
        ti_matrix = TiInterop.to_ti_mat(py_matrix)
        result_str = call_func("abs", ti_matrix)
        return TiInterop.to_py_mat(result_str)
    
    def augment(self, py_matrix1, py_matrix2):
        ti_matrix1 = TiInterop.to_ti_mat(py_matrix1)
        ti_matrix2 = TiInterop.to_ti_mat(py_matrix2)
    ...
    def diag(self, py_list_or_matrix):
        if isinstance(py_list_or_matrix[0], list):  # Assuming it's a matrix
            ti_matrix = TiInterop.to_ti_mat(py_list_or_matrix)
            result_str = call_func("diag", ti_matrix)
        else:  # Assuming it's a list
            ti_list = TiInterop.to_ti_list(py_list_or_matrix)
    ...
    def row_sub(self, input_matrix, row_to_subtract, row_to_target, factor=1):
        ...
        sub_matrix = TiInterop.to_ti_mat(row_subtract_values)
        tar_matrix = TiInterop.to_ti_mat(row_subtract_target)
        
        subtracted_result_str = self.exec("{}-{}*{}".format(tar_matrix, sub_matrix, factor))
        subtracted_result_vec = TiInterop.to_py_mat(subtracted_result_str)[0]
    ...
    def m_row(self, expr, matrix, row_idx):
        ...
        """
        ti_matrix = TiInterop.to_ti_mat(matrix)
    ...
    def get_row(self, py_matrix, row_idx): 
        ti_matrix = TiInterop.to_ti_mat(py_matrix)
        result_str = self.exec("{}[{}]".format(ti_matrix, row_idx))
        return TiInterop.to_py_mat(result_str)
    ...
    def get_col(self, py_matrix, col_idx):
        ...
        result_str = self.exec("subMat", *args)
        return TiInterop.to_py_mat(result_str)
    ...
    def simult(self, py_coeff_matrix, const_col_vector, tol=None):
        ti_coeff_matrix = TiInterop.to_ti_mat(py_coeff_matrix)
        ti_const_vector = TiInterop.to_ti_mat(const_col_vector)
        if tol is not None:
